...
void cSprite::update()
{
	if (m_InputMgr != NULL && isTopPaddle == true)
	{
	...
	if (m_InputMgr != NULL && isBottomPaddle == true)
	{
	...
	if (isBall == true )// you're the ball
	{
		if (spritePos2D.x > 1024.0f - textureWidth || spritePos2D.x < 0.0f )
		{
			isColliding = true;
			hSpeed += 1;
		}
		...
		setSpritePos(glm::vec2(hSpeed*hDirection, vSpeed*vDirection) + spritePos2D);
	}
}